...
public class InvoiceTransformatorImpl implements InvoiceTransformator {
    ...
    public static final String FO_PATH = "src/main/resources/myDocbookFo.xsl";
    
    // url to locate relax ng schema
    public static final String DOCBOOK_SCHEMA_FILE = "src/main/resources/docbook.xsd";
...
//                return url;
    ...
    public String transformToDocbook(Invoice invoice) {
        ...
        try (StringWriter sw = new StringWriter()) {
            ...
            xsw.writeCharacters("Invoice #" + Long.toString(id));
            xsw.writeEndElement(); // end title
            xsw.writeStartElement("pubdate");
            xsw.writeCharacters(invoice.getMonth() + "/" + invoice.getYear());
            xsw.writeEndElement(); // end pubdate
            ...
            xsw.writeEndElement(); // end title
            xsw.writeStartElement("para");
            xsw.writeCharacters(invoice.getEmployer());
            xsw.writeEndElement(); // end para
            xsw.writeEndElement(); // end section
            ...
            for (Entry<Work, Integer> work : invoice.getWorks().entrySet()) {
                ...
                xsw.writeStartElement("entry");
                xsw.writeAttribute("align", "right");
                xsw.writeCharacters(Integer.toString(work.getValue()) + (work.getValue() == 1 ? " hour" : " hours"));
                xsw.writeEndElement(); // end entry
                xsw.writeStartElement("entry");
                xsw.writeAttribute("align", "right");
            ...
            xsw.writeStartElement("row");
            xsw.writeStartElement("entry");
            xsw.writeStartElement("emphasis");
            xsw.writeAttribute("role", "bold");
            xsw.writeCharacters("Total balance");
            xsw.writeEndElement(); // end emphasis
            xsw.writeEndElement(); // end entry
            xsw.writeStartElement("entry");
            xsw.writeAttribute("align", "right");
            xsw.writeStartElement("emphasis");
            ...
            // Specify you want a factory for XML Schema
            SchemaFactory factory
                    = SchemaFactory.newInstance(XMLConstants.W3C_XML_SCHEMA_NS_URI);

            // Compile the schema.
            Schema schema = factory.newSchema(new File(DOCBOOK_SCHEMA_FILE));

            // Get a validator from the schema.
            Validator validator = schema.newValidator();

            // Create reader from created docbook string
            XMLInputFactory xif = XMLInputFactory.newInstance();
            XMLStreamReader xsr = xif.createXMLStreamReader(new StringReader(sw.getBuffer().toString()));
